...
def backtrack_function(currentID, targetID):
    q= Queue()
    q.put([{'room': currentID, "direction": None}])
    visited = set()
    while q.qsize() > 0:
        path = q.get()
        room = path[-1]['room']
        if room not in visited:
            ...
            for next_room in myGraph[room]:
                if myGraph[room][next_room] != '?':
                    new_path = path[:]
                    new_path.append([{'room': myGraph[room][next_room], "direction": next_room}])
                    q.put(new_path)
